...
the current `mog` package (it does *not* run the example, so no network calls
are made).
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Directory passed to `mojo build -I` so `import mog` resolves (default: repo root).",
    )
    parser.add_argument(
        "--mojo",
    ...
    tmp_ctx = tempfile.mkdtemp(prefix="mog_readme_") if args.keep else None
    if args.keep:
        work_dir = Path(tmp_ctx)
        results = _run_all(blocks, mojo_cmd, args.include, work_dir)
        print(f"\nGenerated files kept in: {work_dir}")
    else:
        with tempfile.TemporaryDirectory(prefix="mog_readme_") as tmp:
